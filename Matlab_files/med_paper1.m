...
for i=1:numControllori
    ...
    %if(i>1)
        sistema(i).Kc=sistema(i).Tc'*sistema(i).P*sistema(i).Tc;
        sistema(i).Kp=sistema(i).Tc'*sistema(i).P*sistema(i).Tp;
        sistema(i).Kr=sistema(i).Tc'*sistema(i).P*sistema(i).Tr;
    %end
...
%     Ga=sistema(Jindex_optim).G(1:numStati_c, 1:numStati);
    ...
    x_c_ts=-(sistema(Jindex_optim+1).Kc^-1)*(sistema(Jindex_optim+1).Kp*x_p_ts+sistema(Jindex_optim+1).Kr*x_rd);
    %-(Fa^(-1))*(Fb*x_p_ts-Ga*x_rd);
    
    % stato a ciclo chiuso del prossimo sistema
    x_cc_next=x_cc_optim;
    x_cc_next(1:numStati_c,1)=x_c_ts;

    
%    S=[-sistema(Jindex_optim).O  sistema(Jindex_optim).M];
    xa_optim=[x_cc_optim; x_rd];
    xa_next=[x_cc_next; x_rd];
    xa_normal=[x_cc_normal; x_rd];
    ...
    % manca Im
    K_opt=[-(sistema(Jindex_optim+1).Kc^-1)*sistema(Jindex_optim+1).Kp*B_p*sistema(Jindex_optim).C_c,   -(sistema(Jindex_optim+1).Kc^-1)*sistema(Jindex_optim+1).Kp*(A_p-B_p*sistema(Jindex_optim).D_c*C_p),   -(sistema(Jindex_optim+1).Kc^-1)*(sistema(Jindex_optim+1).Kp*B_p*sistema(Jindex_optim).D_c*Ir+sistema(Jindex_optim+1).Kr*A_rd); 
                                 B_p*sistema(Jindex_optim).C_c,                                                                       A_p-B_p*sistema(Jindex_optim).D_c*C_p,                                      B_p*sistema(Jindex_optim).D_c*Ir;       
                                 zeros(numStati_r,numStati_c),                                                                            zeros(numStati_r,numStati_p),                                                       A_rd                               ];

%     K=[-Fa^-1*Fb*B_p*sistema(Jindex_optim).C_c,  -Fa^-1*Fb*(A_p-B_p*sistema(Jindex_optim).D_c*C_p),  Fa^-1*(Ga*N-Fb*B_p*sistema(Jindex_optim).D_c*Ir);
%         B_p*sistema(Jindex_optim).C_c,                  A_p-B_p*sistema(Jindex_optim).D_c*C_p ,               B_p*sistema(Jindex_optim).D_c*Ir; 
%           zeros(numStati,numStati_c)                         zeros(numStati,numStati_p),                                N                          ];

    
    K_normal =[sistema(Jindex_normal).A_cc,    sistema(Jindex_normal).B_cc*Ir;
            zeros(numStati_r, numStati_c+numStati_p),            A_rd];
    ...
    d_x_cc_optim=sistema(index_optim).A_cc*x_cc_optim+sistema(index_optim).B_cc*x_rd(1,1);
...
Plot1=figure(2);
ax(1)=subplot(2,1,1);
plot(t,x_cc_optim_plot,t,x_cc_normal_plot,t,x_rd_plot(1,:), t, x_cc_system_plot(1,:), t, x_cc_system_plot(2,:), t, x_ed_plot(1,:));
grid on
h=legend('$C_{reset}$','$C_{switch}$', '$\overline{r}$', '$C_1$', '$C_2$', 'ed'); 
