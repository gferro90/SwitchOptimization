function [index_opt, Jmin, Jstore, x_reset]=FindJCostminStore_resetFF(cnt, indexes, numSalti, numStati_a, numStati_c,numStati_p,numStati_r, numStati_w, numeroCampioni, timeMesh, Mc1, Mc2, Aav, x0, Jnow, index_opt, JstoreN, x_reset, Tv, Pv, numStati)
persistent globCnt;
if(isempty(globCnt))
    globCnt=1;
end
...
if(cnt>numSalti)
    ...
    if(invalid)
        ...
        for j=0:numSalti
            MOtemp=MO;
            BOtemp=BO;
            [MO,BO]=SolvePieceFF(ev,  Pv, Tv, j, numSalti, x0, MOtemp,BOtemp, numStati, numStati_a, numStati_c, numStati_p, numStati_r, numStati_w,1);
        end
...
%         -P1cp*x0p-P1cr*x0r-ec'*P2pr*x1r-ec'*P2pp*ep*x0p-ec'*P2pp*er*x0r
%         BO=[-P1cp*x0p-P1cr*x0r-ec'*P2pr*x1r-ec'*P2pp*ep*x0p-ec'*P2pp*er*x0r;
%             -P2cr*x1r-P2cp*x0p-P2cp*er*x0r]
    ...
    for i=1:numeroCampioni
        indexes(cnt)=i;
        cnt=cnt+1;
        [index_opt, Jmin, Jstore, x_reset]=FindJCostminStore_resetFF(cnt, indexes, numSalti, numStati_a, numStati_c,numStati_p,numStati_r, numStati_w, numeroCampioni, timeMesh, Mc1, Mc2, Aav, x0, Jmin, index_opt, Jstore, x_reset, Tv, Pv, numStati);
        cnt=cnt-1;
    end
end
end

% function [index_opt, Jmin, Jstore, x_reset]=FindJCostminStore_resetFF(cnt, indexes, numSalti, numStati_a, numStati_c,numStati_w, numeroCampioni, timeMesh, Mc1, Mc2, Aav, x0, Jnow, index_opt, JstoreN, x_reset, Tv, Pv, numStati)
% persistent globCnt;
% if(isempty(globCnt))
%     globCnt=1;
% end
...
%         x_o=[xc_o; xnc; xw_o];
%         xx(numStati_a*(j-1)+1:j*numStati_a,1)=xi;
%         xo(numStati_a*(j)+1:j*numStati_a*(j+1),1)=x_o;
%     end
%     if(invalid)
%         J=Inf;
%     else
...
%         indexes(cnt)=i;
%         cnt=cnt+1;
%         [index_opt, Jmin, Jstore, x_reset]=FindJCostminStore_resetFF(cnt, indexes, numSalti, numStati_a, numStati_c,numStati_w, numeroCampioni, timeMesh, Mc1, Mc2, Aav, x0, Jmin, index_opt, Jstore, x_reset, Tv, Pv, numStati);
%         cnt=cnt-1;
%     end
% end
% end