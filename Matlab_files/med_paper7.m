...
%% DEFINIZIONE TEMPO DI SIMULAZIONE
 ...
 %ottimizzazione offline con reset ottimo
...
for i=1:numControllori
    ...
    sistema(i).A_a=[sistema(i).A_cc,               sistema(i).B_cc*Ir;
        zeros(numStati_r,numStati_cc),                   A_rd];
    ...
    sistema(i).T=[-sistema(i).O, (eye(numStati_v)-sistema(i).M1)*Nr];

  
    
    sistema(i).A_ed=A_ed_temp;
...
numStati_a=numStati_cc+numStati_r;
...
if(numSalti==1)
    ...
    [index_opt, Jmin, Jstore, x_reset]=FindJCostminStore_reset(cnt, indexes, numSalti, numStati_a, numStati_c, numStati_p,numStati_r,numeroCampioni, timeMesh, Mc1, Mc2, Aav, CI_a, Jnow, indexes, Jstore, CI_a, Tv, Pv, numStati);
    plot(t,Jstore);
    ...
    if(numSalti==2)
        ...
        [index_opt, Jmin, Jstore, x_reset]=FindJCostminStore_reset(cnt, indexes, numSalti, numStati_a, numStati_c, numeroCampioni, timeMesh, Mc1, Mc2, Mc3, Aav, CI_a, Jnow, indexes, Jstore, CI_a);
        ...
        [index_opt, Jmin, x_reset]=FindJCostmin_reset(cnt, indexes, numSalti, numStati_a, numStati_c, numeroCampioni, timeMesh, Mc1, Mc2, Mc3, Aav, CI_a, Jnow, indexes, CI_a)
...
CI_a_new=x_reset(1:numStati_a,1);
...
x_a=CI_a_new;
x_a_plot=zeros(numStati, samples);
...
for i=1:samples
    ...
    x_rd_plot(:,i)=x_rd;
    x_ed_plot(:,i)=x_ed;
    x_a_plot(:,i)=sistema(index_normal).T*x_a;
    ...
    if(switch_i<=numSalti)
        if(t1(i)>=t(index_opt(switch_i)))
            ...
            switch_i=switch_i+1;
            x_ed=sistema(index_normal).T*x_a_normal;
            x_a=x_a_normal;
        end
    end
...
Plot1=figure(2);
%ax(1)=subplot(2,1,1);
plot(t1,x_cc_normal_plot,t1,x_rd_plot(1,:), t1, x_cc_system_plot(1,:), t1, x_cc_system_plot(2,:), t1, x_ed_plot(1,:), t1,x_a_plot(1,:));
grid on
